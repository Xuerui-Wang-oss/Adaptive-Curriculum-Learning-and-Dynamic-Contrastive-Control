...
class her_sampler:
    ...
    def sample_her_contrastive_transitions(self, episode_batch, batch_size_in_transitions, 
        ...
        # 优先使用 sac_agent 预计算的缓存分数（每 cycle 只算一次），
        # 否则回退到完整 LSTM 推理（兼容直接调用的场景）
        if 'contrastive_scores' in episode_batch:
            scores = episode_batch['contrastive_scores']
            ...
            with torch.no_grad():
                ...
                if self.goal_type == 'full':
                    trajectories = torch.tensor(
                        episode_batch['ag'][:, key_frames, :], dtype=torch.float32
                    )
                elif self.goal_type == 'rotate':
                    trajectories = torch.tensor(
                        episode_batch['ag'][:, key_frames, 3:], dtype=torch.float32
                    )
                else:
                    raise ValueError("Invalid goal_type. Choose 'full' or 'rotate'.")

                lambda_tensor = torch.tensor(
                    [[current_lambda_val]] * rollout_batch_size, dtype=torch.float32
                )
                encoder_device = next(self.trajectory_encoder.parameters()).device
                if encoder_device.type == 'cuda':
                    encodings = self.trajectory_encoder(
                        trajectories.cuda(), lambda_tensor.cuda()
                    )
