...
def info_nce_loss(positive_encodings, negative_encodings, temperature=0.1):
    ...
    For each anchor pos[i], the positive key is pos[i] itself.
    This pushes each positive encoding to be distinguishable from all
    other encodings (both other positives and all negatives), which
    combined with norm_loss results in high-quality trajectories having
    large, spread-out representations used for score-based sampling.

    Parameters:
    - positive_encodings: High-quality trajectory encodings [N, encoding_dim]
    - negative_encodings: Low-quality trajectory encodings [M, encoding_dim]
    ...
    pos_norm = F.normalize(positive_encodings, dim=1)  # [N, D]
    neg_norm = F.normalize(negative_encodings, dim=1)  # [M, D]

    # Similarity of each pos[i] against [all positives, all negatives]
    all_encodings = torch.cat([pos_norm, neg_norm], dim=0)  # [N+M, D]
    similarity_matrix = torch.matmul(pos_norm, all_encodings.T) / temperature  # [N, N+M]

    # label i → all_encodings[i] = pos_norm[i] (self as positive key)
